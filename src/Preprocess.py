...
class Preprocess:
    ...
    def __add_mask(self, n):
        ...
        self.segs = self.segs + ([self.bert_tokenizer.vocab['[MASK]']] * pad_len)
        self.mask = ([True] * (n-pad_len)) + ([False] * pad_len)
        self.mask_cls = [True] * len(self.clss)
